...
class EnhancedReportGenerator:
    ...
    def initialize_engines(self):
        ...
        if REPAIR_MODULE_OK:
            self.repair_ranker = RepairRankingEngine(
                topology_graph=getattr(self.topology_graph, 'graph', self.topology_graph),
                device_map=self.device_map,
                switch_status_map=self.switch_status_map,
            )
    ...
    def run_physical_constraint_check(self) -> PhysicalConstraintSummary:
        ...
        # 物理约束校验：由 run_batch_check 从 topology_graph 推导真实节点
        # （KCL 取关联≥2设备的连接点；支路校验用真实端子，不传假节点 NODE_A/B）
        self.physical_checker.run_batch_check(
            nodes_to_check=None,
            switches_to_check=None,
            topology_graph=self.topology_graph,
    ...
    def _process_single_defect(self, defect: dict, idx: int) -> EnhancedDefectReport:
        ...
        if PHYSICAL_MODULE_OK and self.physical_checker and self.topology_graph is not None:
            ...
            try:
                ...
                _g = getattr(self.topology_graph, 'graph', self.topology_graph)
                pts = [n for n in _g.neighbors(equip_id)]
