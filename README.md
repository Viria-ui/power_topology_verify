...
>
> 设计与历史版本说明见下方 [6.5 v3.0 单线路全流程 — 更新说明](#65-v30-单线路全流程--更新说明)。
...
#### 6.5.3 完整产物清单（`--line LINE111 --hybrid --repair` 一次跑出来）
...
| 10kVLINE018 | 383 | 167→355 | 0 | 61→3（+46.9分）|

> 2026-09-10 P1-P4 修复后测试结果：
> - P1修复：ZWLINEEND 映射数 0→149（线路名称标准化匹配）
> - P2修复：电气缺陷过滤 3554→299（按 feeder_id 过滤全网缺陷）
> - P3修复：拓扑构建单次执行（消除重复构建，时间节省约50%）
> - P4修复：美化质量 39.6→86.5（重叠阈值采用绝对尺寸 MIN_DEVICE_SIZE=15.0，避免美化SVG缩放误报）
